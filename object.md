...
cart.wheels = 3;
cart.wheels; // Uncaught Error: 数值太小了！
```
...
obj.foo.get.name // TypeError: Cannot read properties of undefined (reading 'get')
obj.foo.set.name // TypeError: Cannot read properties of undefined (reading 'set')

const descriptor = Object.getOwnPropertDyescriptor(obj, 'foo');
...
有两种特殊情况：`bind`方法创造的函数，`name`属性返回`bound`加上原函数的名字；c。
...
console.log(Object.getOwnPropertyDescriptor(obj, "foo2"));
console.log(Object.getOwnPropertyDescriptor(obj, "foo3"));
// {value: 123, writable: true, enumerable: true, configurable: true}
// {value: 'foo2', writable: true, enumerable: true, configurable: true}
// {value: 'foo3', writable: true, enumerable: true, configurable: true}
```
...
let { ...x, y, z } = { x: 1, y: 2, a: 3, b: 4 };    // Rest element must be last element
let { x, ...y, ...z } = { x: 1, y: 2, a: 3, b: 4 }; // Rest element must be last element
```
...
let { ...x } = obj;
obj.a.b = 2;
console.log(x.a.b); // 2 浅复制，复制的是这个值的引用，而不是这个值的副本
```
...
let { ...o3 } = o2;
console.log(o3); // {b: 2}
console.log(o3.a); // undefined 解构赋值不会复制继承自原型对象的属性
```
...
// P174
function baseFunction({ a, b }) {
  console.log("baseFunction a", a); // a
  console.log("baseFunction b", b); // b
}
function wrapperFunction({ x, y, ...restConfig }) {
  // 使用 x 和 y 参数进行操作
  // 其余参数传给原始函数
  console.log("wrapperFunction x", x); // x
  console.log("wrapperFunction y", y); // y
  console.log("wrapperFunction restConfig", restConfig); // {a: 'a', b: 'b', c: 'c', d: 'd'}
  return baseFunction(restConfig);
}
let params = {
  x: "x",
