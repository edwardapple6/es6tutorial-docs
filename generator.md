...
var hw = helloWorldGenerator(); // 调用 Generator 函数，返回一个遍历器对象，代表 Generator 函数的内部指针
console.log(hw);
// helloWorldGenerator {<suspended>}
//   [[GeneratorLocation]]: VM414:1
//   [[Prototype]]: Generator
...
// Uncaught TypeError: (intermediate value)(intermediate value).next is not a function
...
// 而使用这个方法的时候，系统必须知道你到底要终止的是哪一个setTimeout()方法(因为你可能同时调用了好几个 setTimeout()方法)，
// 这样clearTimeout()方法就需要一个参数，这个参数就是setTimeout()方法的返回值(数值)，用这个数值来唯一确定结束哪一个setTimeout()方法。
// clearTimeout（timeid），会清除掉这个定时函数，但是不会改变id值。



/**
 * 如果是普通函数，在为变量`generator`赋值时就会执行。
 */
function f() {
  console.log("执行了！");
}
...
var flat = function* (a) {
  ...
  for (var i = 0; i < length; i++) {
    var item = a[i];
    if (typeof item !== 'number') {
      console.log("item 非 number:", item);
      yield* flat(item);
    } else {
      yield item;
    }
...
console.log(g.next());     // reset undefined   // {value: 1, done: false}
console.log(g.next());     // reset undefined   // {value: 2, done: false}
console.log(g.next());     // reset undefined   // {value: 3, done: false}
console.log(g.next());     // reset undefined   // {value: 4, done: false}
console.log(g.next());     // reset undefined   // {value: 5, done: false}
console.log(g.next(true)); // reset true   // {value: 0, done: false}
```
...
var b = foo(5);
b.next(b.next().value) // {value: 4, done: false}
```
...
let tem = foo();
console.log(tem.next()); // {value: 1, done: false}
console.log(tem.next()); // {value: 2, done: false}
console.log(tem.next()); // {value: 3, done: false}
console.log(tem.next()); // {value: 4, done: false}
console.log(tem.next()); // {value: 5, done: false}
console.log(tem.next()); // {value: 6, done: true}
...
F(0)=0，F(1)=1, F(n)=F(n - 1)+F(n - 2)（n ≥ 2，n ∈ N*） // 数学上，斐波那契数列以递推的方法定义
...
// Uncaught TypeError: jane is not iterable

function* objectEntries(obj) {
  let propKeys = Reflect.ownKeys(obj); // Reflect.ownKeys (target) 用于返回对象的所有属性
...
let numb = numbers();
console.log(numb.next()); // {value: 1, done: false}
console.log(numb.next()); // {value: 2, done: false}
console.log(numb.next()); // {value: 3, done: true}
console.log(numb.next()); // {value: undefined, done: true}
